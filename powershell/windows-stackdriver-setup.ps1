...
Write-Host "Installing StackdriverLogging"
Invoke-webrequest https://dl.google.com/cloudagents/windows/StackdriverLogging-v1-10.exe -OutFile ".\StackdriverLogging-v1-10.exe";
...
elseif ($function -eq "sdc"){
    Write-Host "Setting windows-startup-script-url in metadata to $GcsPrefix/powershell/bootstrap/secondary-domain-controller-step-1.ps1"
    gcloud compute instances add-metadata "$name" --zone $zone --metadata windows-startup-script-url="$GcsPrefix/powershell/bootstrap/secondary-domain-controller-step-1.ps1"
}
